...
import pickle
import time
from mongodb_util import save_posture_data
import sys
import signal  # Add the signal import
...
for attempt in range(retry_count):
    try:
        ...
        print("Connected to the webcam server!")
        break
    except ConnectionRefusedError:
        print(f"Connection refused. Retrying in {retry_delay} seconds... (Attempt {attempt + 1}/{retry_count})")
        time.sleep(retry_delay)
else:
    print("Failed to connect to the webcam server after multiple attempts.")
    sys.exit(1)
